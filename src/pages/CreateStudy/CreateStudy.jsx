...
const CACHE_KEY = "createStudyCache";
...
const CreateStudy = () => {
  ...
  useEffect(() => {
    ...
    const cached = loadFromCache(currentUser.uid);
    if (cached) {
      setFormData(cached.formData || formData);
      if (cached.uploadedFile) {
        setUploadedFile(cached.uploadedFile);
      }
      setAnalysisId(cached.analysisId || null);
      setAnalyzedDocumentName(cached.analyzedDocumentName || "");
    }
  }, [currentUser]);

  useEffect(() => {
    if (!currentUser || !uploadedFile) return;

    const timeoutId = setTimeout(() => {
      saveToCache(currentUser.uid, {
        formData,
        uploadedFile,
        analysisId,
        analyzedDocumentName,
      });
    }, 1000);

    return () => clearTimeout(timeoutId);
  }, [formData, uploadedFile, analysisId, analyzedDocumentName, currentUser]);

  const pollAnalysisStatus = useCallback(
    async (analysisId, token) => {
      const interval = setInterval(async () => {
        try {
          const response = await fetch(
            `/api/studies/analysis-status/${analysisId}`,
            {
          ...
          if (response.ok && data.status === "completed") {
            ...
            addNotification(
              "Document analysis complete! Click here to view the results.",
              "success",
              "/create-study"
            );
  ...
  const handleAnalyzeDocument = async (file) => {
    ...
    try {
      ...
      addNotification(
        "Document analysis started. We'll notify you when it's complete.",
        "info"
      );
